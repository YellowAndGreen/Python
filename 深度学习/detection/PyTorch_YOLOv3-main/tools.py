...
class MSELoss(nn.Module):
    ...
    def forward(self, logits, targets, mask):
        inputs = torch.clamp(torch.sigmoid(logits), min=1e-4, max=1.0 - 1e-4)  # clamp控制数值范围
        ...
        total_loss = 5.0 * pos_loss + 1.0 * neg_loss

        # 若模式为均值，则全部相加后除以批量数
        if self.reduction == 'mean':
            batch_size = logits.size(0)
            total_loss = torch.sum(total_loss) / batch_size
...
def multi_gt_creator(input_size, strides, label_lists, anchor_size):
    """
    制作训练正样本
    注意：这里的多尺度是指在多尺度上预测，而不是多尺度技巧（改变输入图片的尺度）
    input_size => 输入的图片大小
    stride => 降采样倍数
    label_lists => [B,num_labels,5] => [xmin, ymin, xmax, ymax, label_ind]
    anchor_size => [9,2] => 九个锚框，每个锚框的长和宽，在data/config.py中设定
    """
    batch_size = len(label_lists)
    h = w = input_size
    num_scale = len(strides)
    all_anchor_size = anchor_size
    anchor_number = len(all_anchor_size) // num_scale  # 3
    # [3,batch_size,hs,ws,anchor_number, 1 + 1 + 4 + 1 + 4]
    # 正样本标签向量，分别对应着每个边界框的置信度（1）、类别序号（1）、边界框位置参数（4）、边界框的权重（1）以及真实框的坐标信息（4）
    gt_tensor = []
    for s in strides:
            ...
            if iou_mask.sum() == 0:
                ...
                for index, iou_m in enumerate(iou_mask):
                    if iou_m:
                        if index == best_index:
                            # 确定该正样本被分配到哪个尺度上去，以及哪个先验框被选中为正样本
                            # s_indx表示尺度类别，ab_ind表示尺度上的先验框类别
                            s_indx = index // anchor_number  # index为0-8，anchor_number为3
                            ab_ind = index - s_indx * anchor_number
                            # 获得该尺度的降采样倍数
                            s = strides[s_indx]
                            # 获得该先验框的参数
                            ...
                            tx = c_x_s - grid_x
                            ty = c_y_s - grid_y
                            tw = np.log(box_w / p_w)  # 网格尺度 / 锚框长度
                            th = np.log(box_h / p_h)
                            weight = 2.0 - (box_w / w) * (box_h / h) # 锚框大小权重，大尺寸的锚框本身能够有更大的权重，所以需要降低权重
    ...
    # [batch_size,hs,ws,anchor_number, 1 + 1 + 4 + 1 + 4] => [batch_size,3*hs*ws*anchor_number, 1 + 1 + 4 + 1 + 4]
...
def iou_score(bboxes_a, bboxes_b):
    """
        ...
        测试：
        bboxes_a = torch.tensor([1,1,4,4]).unsqueeze(0)
        bboxes_b = torch.tensor([2,3,5,5]).unsqueeze(0)
        tl,br,tl < br,en,torch.prod(br - tl, 1),area_i,area_a,area_b
    """
    # 左下点和右上点
...
def loss(pred_conf, pred_cls, pred_txtytwth, pred_iou, label):
    ...
    pred_cls = pred_cls.permute(0, 2, 1)  # [B, H*W*anchor_n*3, num_cls]
    ...
    gt_mask = (gt_box_scale_weight > 0.).float()  # 对于不需要预测的框设置为0
