...
class SENet(nn.Module):
    ...
    def __init__(self, channel, ratio=4):
        ...
        self.Linear1 = nn.Linear(channel, channel // ratio, False)
        self.Linear2 = nn.Linear(channel // ratio, channel, False)

    def forward(self, x):
        b, c, h, w = x.size()
        avg = self.avg_pool(x).view([b, c])  # b,c,h,w->b,c
        fc = torch.relu(self.Linear1(avg))
        fc = torch.sigmoid(self.Linear2(fc))
        fc = fc.view([b, c, 1, 1])  # b,c->b,c//ratio->b,c->b,c,1,1
        # 残差块内的输出乘上通道注意力得出的向量
        return x * fc
...
class resblock(nn.Module):
    ...
    def forward(self, x):
        # 在此将通道注意力权重加在module(x)上
        for module in self.module_list:
            x = self.chnet(module(x)) + x
        return x
