...
class YOLOv3(nn.Module):
    def __init__(self, device, input_size=None, num_classes=20, trainable=False, conf_thresh=0.001, nms_thresh=0.50,
        ...
        self.stride = [8, 16, 32]  # 在三个尺度的特征图上做预测
        self.anchor_size = torch.tensor(anchor_size).view(3, len(anchor_size) // 3, 2)   # 先验框的个数
    ...
    def create_grid(self, input_size):
        ...
        for ind, s in enumerate(self.stride):   # 生成[8, 16, 32]三个尺度的网格
            # generate grid cells
            ws, hs = w // s, h // s
            ...
            stride_tensor = torch.ones([1, hs * ws, self.num_anchors, 2]) * s  # ？？为什么这里要乘上s

            # generate anchor_wh tensor
            anchor_wh = self.anchor_size[ind].repeat(hs * ws, 1, 1)  # 每个网格都有一个像素点：[hs*ws, anchor_w, anchor_h]
        ...
        # 三个尺度进行合并
        total_grid_xy = torch.cat(total_grid_xy, dim=1).to(self.device)   # [3,hs*ws,1,2]
        total_stride = torch.cat(total_stride, dim=1).to(self.device)
        total_anchor_wh = torch.cat(total_anchor_wh, dim=0).to(self.device).unsqueeze(0)   # [1,3*hs*ws, anchor_w, anchor_h]
    ...
    def forward(self, x, target=None):
        ...
        for pred in preds:
            ...
            # 每个位置预测n个锚框，每个锚框有C个预测参数（置信度，类别，锚框位置）
            # [B, anchor_n * C, H, W] -> [B, H, W, anchor_n * C] -> [B, H*W, anchor_n*C]
            # 因为三个预测方向的H*W不一样，不能直接合并，所以直接将HW放在一个维度就可以合并了
            ...
            cls_pred = pred[:, :, 1 * self.num_anchors: (1 + self.num_classes) * self.num_anchors].contiguous().view(B_,
                                                                                                                     H_ * W_ * self.num_anchors,
                                                                                                                     self.num_classes)
            # [B, H*W*anchor_n, 4]
        ...
        if self.trainable:
            ...
            # 从txtytwth预测中解算出x1y1x2y2坐标   decode_boxes函数不改变维度  [B, H*W*anchor_n, 4] => [B*H*W*anchor_n, 4]
            x1y1x2y2_pred = (self.decode_boxes(txtytwth_pred) / self.input_size).view(-1, 4)

            # [batch_size,3*hs*ws*anchor_number, 1 + 1 + 4 + 1 + 4]
            x1y1x2y2_gt = target[:, :, 7:].view(-1, 4)
            # 计算pred box与gt box之间的IoU    [B*H*W*anchor_n, 4] => [B,H*W*anchor_n, 1]
            ...
            # 我们将pred box与gt box之间的iou作为objectness的学习目标.
            # 这里将iou添加到目标中作为置信度，由于维度是[B,H*W*anchor_n, 1]，所以添加到第三维
