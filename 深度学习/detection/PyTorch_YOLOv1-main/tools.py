...
def generate_dxdywh(gt_label, w, h, s):
    ...
    tw = np.log(box_w)
    th = np.log(box_h)
    # 计算边界框位置参数的损失权重 [1-2]之间
    weight = 2.0 - (box_w / w) * (box_h / h)

    return grid_x, grid_y, tx, ty, tw, th, weight


# 根据真实框的参数列表生成真实框的标签值（与网络输出维度相同）
...
def loss(pred_conf, pred_cls, pred_txtytwth, label):
    # 损失函数
    conf_loss_function = MSEWithLogitsLoss(reduction='mean')   # 上面所定义
    cls_loss_function = nn.CrossEntropyLoss(reduction='none')
    # torch.nn.BCEWithLogitsLoss函数中已经自带了sigmoid功能
    # 为什么这里要用BCE？这个不是分类用的吗？还是说仅为了做sigmoid
    txty_loss_function = nn.BCEWithLogitsLoss(reduction='none')
    twth_loss_function = nn.MSELoss(reduction='none')

    # 预测
    pred_conf = pred_conf[:, :, 0]
    # 类别预测   [B, H*W, num_cls]  ==>>  [B, num_cls, H*W]
    ...
    # 标签
    gt_obj = label[:, :, 0]
    gt_cls = label[:, :, 1].long()  # 又转回为整数
    gt_txty = label[:, :, 2:4]
    gt_twth = label[:, :, 4:6]
    gt_box_scale_weight = label[:, :, 6]  # 缩放权重

    batch_size = pred_conf.size(0)
    # 置信度损失
    # gt_obj是真实框的置信度
    # 应该是锚框与真实框的IOU,这里是为简便
    conf_loss = conf_loss_function(pred_conf, gt_obj)

    # 类别损失
    # gt_cls是0或1，gt_obj是当前类的独热向量
    # 这里乘上gt_obj表示不负责预测的锚框不需要计算损失，下面同理
